...
class App extends Component {
  ...
  onIsVistedChange = id => {
    ...
    const updatedCountriesList = countriesList.map(eachItem => {
      if (eachItem.id === id) {
  ...
  onRemoveItem = id => {
    ...
    this.setState({countriesList: filteredCountriesList})
  }
      ...
      <div className="app-container">
        ...
        <ul className="unordered-list">
          {countriesList.map((eachItem, index) => (
            <EachCard
              key={eachItem.id}
              eachContent={eachItem}
              lastItemBorderBottom={index === initialCountriesList.length - 1}
              onIsVistedChange={this.onIsVistedChange}
            />
          ))}
        </ul>
        ...
        {visitedCountries.length === 0 ? (
          <div className="not-found-container">
            <p className="not-found-name">No Countries Visited Yet!</p>
          </div>
        ) : (
          <ul className="visited-unordered-list">
